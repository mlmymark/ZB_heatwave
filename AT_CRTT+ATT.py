# -*- coding: utf-8 -*-
"""
体感温度热浪事件识别：固定阈值与气候逐日百分位阈值，指定经纬度。

高温日须同时达到固定温度阈值与气候逐日百分位阈值（CRTT）。
该阈值按同一气候日跨年计算，闰年 2 月 29 日的样本并入 2 月 28 日。
...
def date_to_doy(date):
    """返回 0—364 的气候学日序；闰年 2 月 29 日并入 2 月 28 日。"""
    doy = date.timetuple().tm_yday
    if calendar.isleap(date.year) and (date.month > 2 or (date.month == 2 and date.day == 29)):
        doy -= 1
    return doy - 1


def compute_daily_percentile_threshold(series, dates, percentile):
    """使用全部输入年份的同气候日样本计算百分位阈值，以单精度浮点数存储。"""
    ...
    doy_to_indices = {}
    for index, date in enumerate(dates):
        doy_to_indices.setdefault(date_to_doy(date), []).append(index)
    thresholds = np.full(365, np.nan, dtype=np.float32)
    for doy, indices in doy_to_indices.items():
...
def detect_events(series, dates, start_year, end_year, temp_threshold,
    ...
    """逐年识别事件；组合阈值模式须同时达到气候逐日阈值和固定温度阈值。"""
    if method not in ('absolute', 'relative'):
    ...
    if method == 'relative':
        print(f"正在用输入目录全部日期计算 {percentile}% 分位数 CRTT...")
        # 先使用输入目录的全部年份建立气候基准，再截取需要输出的年份。
        daily_thresholds = compute_daily_percentile_threshold(values, dates, percentile)
    ...
    for year in range(start_year, end_year + 1):
        ...
        if daily_thresholds is not None:
            ...
            # 先保留气候逐日阈值的单精度存储方式，再以双精度比较，避免额外降精度。
            thresholds = np.maximum(daily_thresholds[doys].astype(np.float64),
                                    float(temp_threshold))
        year_events = find_heatwave_events(year_series, year_dates, thresholds,
                                          duration_threshold)
...
if __name__ == "__main__":
    ...
    PERCENTILES = (90, 95)  # 分别计算第 90、95 百分位方案；单组可写 (90,)
    # ============================================
    if METHOD == "absolute":
