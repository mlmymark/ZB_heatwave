# -*- coding: utf-8 -*-
"""
地表温度热浪事件识别：固定阈值与气候逐日百分位阈值，指定经纬度。
...
THRESHOLD_KIND = "CRTT"  # 阈值类型：气候逐日百分位阈值
DEFAULT_METHOD = "relative"  # 默认判定模式
...
if __name__ == "__main__":
    ...
    BASELINE_START_YEAR = 2003  # 气候逐日阈值基准期的起始年份
    BASELINE_END_YEAR = 2024  # 基准期终止年份；起止均为 None 时使用全部输入年份
    BREAK_ON_DATE_GAP = True  # 普通缺日是否中断事件；设为 False 时可按影像序号跨缺日连接
    # ==============================================
    for percentile in (PERCENTILES if METHOD == "relative" else (90,)):
        main(
            ...
            break_on_date_gap=BREAK_ON_DATE_GAP,
